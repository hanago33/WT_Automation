...
import wt_dpi
import generic_flow_executor as wt_flow_executor
import generic_flow_locator as wt_flow_locator
...
def _apply_param_table_expansion(merged_payload, target_payload):
	...
	# 优先于参数表模板默认值。仅当 runtime 有非空值时才覆盖，避免污染既有行为。
...
def _get_main_window_candidates():
	...
	try:
		main_windows = _get_wt_window_helpers().find_main_windows(
			...
			class_name_keywords=TARGET_CLASS_KEYWORDS,
		)
...
def _auto_capture_template(step_id, step_definition):
    ...
    try:
        ...
        import pyautogui
        import generic_flow_locator as flow_locator
    except Exception:
        return None
...
def _attach_mup_data_diff(run_report, context):
	...
	try:
		...
		if result["newCount"] or result["changedCount"] or result["deletedCount"]:
			...
			log_step(
				"[data] 运行后检测到数据文件变化: 新增{new} 修改{changed} 删除{deleted}".format(
...
def _preflight_check_main_window():
	...
	try:
		...
		raise RuntimeError(
			...
			% (MAIN_WINDOW_TITLE_RE.pattern, GM_EXE or "目标软件", exc)
		) from exc
...
def _pre_raise_main_window(timeout_seconds=15):
	...
	try:
		...
		log_step(
			"[preflight] 运行前置顶完成: hwnd={hwnd} title={title} size={width}x{height}".format(
				**window_info
			)
		)
		return window_info
	except Exception as exc:
		log_step(f"[preflight] 运行前置顶未生效（继续执行）: {exc}")
		return None
