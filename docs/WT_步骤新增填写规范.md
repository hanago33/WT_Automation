...
- 适用场景：MBAElementConfigurationView 测风点/风机/绘图类别展开后的候选批量选择（纯 TextBlock 项，无 UIA 选择模式）。
- 推荐动作：`select_list_items`
- 推荐定位：`automation_id,control_type,name`
- 推荐目标值：`ElementConfigurationView_Label_Category,Text,<类别名>`（类别锚）

建议填写：

- `步骤名称`：如 `选择-测风点候选项`
- `text`：候选名单，**逗号分隔**；留空或 `ALL` = 全选
- 名单可注入：如 `${runtime.mastIdsJoined}`（项目文件夹自动解析）

关键提醒：

- 类别锚点击展开后才能枚举候选，前一步必须是"点击类别"
- 名单精确匹配，未命中任一项时步骤判失败；未解析占位符自动按全选兜底
- 候选项多于可视区时自动逐项滚动，无需加滚动步骤
- 候选选中后需紧跟一步 `click` 点 `>>`（ElementConfigurationView_Button_LeftToRight）

口诀：

- 批量候选先点类别展开，锚用 Category，名单空即全选
